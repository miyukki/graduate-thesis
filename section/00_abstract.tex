...
4K映像の帯域は2K映像と比較すると約4倍、現行のハイビジョン放送と比較すると約8倍にもなり、伝送方法とそのコストが課題である。そのため、映像業界では、映像機器と比べて比較的安価なネットワークリソースを活用し、ネットワークを介して伝送する{\tt Video over IP}化が進んでいる。
...
評価として、4K非圧縮映像の伝送を行う既製品であるPFU QG70、実装したソフトウェア、実装したハードウェアのそれぞれにおいて遅延、重さ、XXXを計測し、XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXという結果となった。
