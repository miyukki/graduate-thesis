...
% 日本語情報（必要なら）
\jclass   {卒業論文}                             % 論文種別
\jtitle   {映像制作現場における\\高解像度映像IP伝送システムNG-HDMI-TSの実装と評価}    % タイトル。改行する場合は\\を入れる
\juniv    {慶應義塾大学}                  % 大学名
...
\etitle   {Implementation and Evaluation of Delivery System for High Resolution Video at Video Production Site}      % タイトル。改行する場合は\\を入れる
...
\include{section/02_video_transmission}
\include{section/03_network_transmission}
\include{section/04_software_experimentation}
\include{section/05_implementation}
\include{section/06_evaluation}
\include{section/07_conclusion}
